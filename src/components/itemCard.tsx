...
export const ItemCard = ({
  item: { id, title, imageUrl, price, discountedPrice },
  detailsLink = true,
}: {
  item: ProductInfo;
  detailsLink?: boolean;
    ...
    <Card>
      <CardTitle>
        <div className="h-96 relative">
          ...
          />
          {isDiscounted ? (
            <div className="w-36 bottom-0 left-0 bg-red-500 text-white absolute p-2">
              Take {discountPercent}% off
            </div>
          ) : null}
        </div>
      </CardTitle>
      <CardContent className="h-32">
        <div>
          ...
          {isDiscounted ? (
            <div className=" text-lg line-through text-red-500">{price},-</div>
          ) : null}
